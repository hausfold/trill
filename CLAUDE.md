...
| Worktree hooks | `~/.claude/settings.json` (yours, not the repo's) → `holt hook create` / `holt hook remove` | Declared by haus and re-asserted every rebuild, so they self-heal when Claude rewrites that file. No repo in the family hand-edits it. |
