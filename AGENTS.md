...
| the mark — app icon, README banner | here: `assets/trill-icon-master.svg`, `assets/trill-square-inverted.svg` and `assets/trill-square-latte.svg` (the light tile, in nebelung's latte set) are the three marks' **sources of record**, their PNGs render from them, `assets/trill-banner.png` has no source here and is redrawn from the brand kit. nebelung's `yellow` / `surface0` / `surface1` / `surface2`, `mantle` in the inverted tile, and `base` plus the whole latte ramp in the light tile, are baked into the PNGs, so a palette change is swapped in **all three** SVGs, then re-rendered down through the master and the `Trill/Assets.xcassets/AppIcon.appiconset` slots ([`assets/README.md`](./assets/README.md)). `~/.config/trill/theme.json` never reaches them. Keep the icon flat; macOS 26 adds inset, shadow, gloss |
