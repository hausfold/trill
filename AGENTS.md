...
| trill's **mark** — the app icon in Finder, Login Items and every Privacy row | here, `assets/trill-icon-master.png`. The three hexes in it are nebelung's `sky` / `surface0` / `surface1` baked into a PNG, so a palette change does **not** reach them — re-render the master and the ten `Trill/Assets.xcassets/AppIcon.appiconset` slots from it (recipe in [`assets/README.md`](./assets/README.md)). The icon is the one trill surface `~/.config/trill/theme.json` cannot retint |
