...
| the mark — app icon, README banner | here: `assets/trill-icon-master.svg` is the mark's **source of record**, `assets/trill-icon-master.png` renders from it, `assets/trill-banner.png` has no source here and is redrawn from the brand kit. nebelung's `yellow` / `surface0` / `surface1` / `surface2` are baked into the PNGs, so a palette change is swapped in the SVG, then re-rendered down through the master and the `Trill/Assets.xcassets/AppIcon.appiconset` slots ([`assets/README.md`](./assets/README.md)). `~/.config/trill/theme.json` never reaches them. Keep the icon flat; macOS 26 adds inset, shadow, gloss |
