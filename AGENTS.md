...
| trill's **mark** — the app icon in Finder, Login Items and every Privacy row | here, `assets/trill-icon-master.png`. The four hexes in it are nebelung's `yellow` / `surface0` / `surface1` / `surface2` baked into a PNG, so a palette change does **not** reach them — re-render the master and the ten `Trill/Assets.xcassets/AppIcon.appiconset` slots from it (recipe in [`assets/README.md`](./assets/README.md)). The icon is the one trill surface `~/.config/trill/theme.json` cannot retint, and it is deliberately flat: macOS 26 adds the inset, shadow and gloss itself |
