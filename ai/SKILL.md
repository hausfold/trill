...
Everything above is one-way. `trill ask` **blocks** until a pill is pressed and
**exits with that pill's index** (0 = the first `--pill`) — a permission prompt
of yours becomes a banner, and the answer comes back to you.
...
- **A file the user needs to grab.** That's `perch add`.
