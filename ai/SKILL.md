...
| 1 | bad usage, including a verb trill doesn't know | fix the flags or the verb; `--title` is required unless `--json`. `trill help` lists every verb — one that should be there means this Trill is older than the skill |
| 2 | daemon unreachable | Trill isn't running — tell the user to launch it |
| 3 | refused: bad JSON, empty title, `skill install` leaving a file that differs alone (a symlink haus manages is not that: exit 0) | fix the call, or update Trill |
